<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Rediģēt komandu: {{ $team->name }}</h2>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto px-4 py-8">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <form method="POST" action="{{ route('teams.update', $team) }}" enctype="multipart/form-data"
                  x-data="teamForm({{ old('players_per_team', $team->players_per_team) }}, {{ json_encode(old('players', $team->players)) }})">
                @csrf @method('PUT')
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <input name="coach" value="{{ old('coach', $team->coach) }}"
                               class="w-full rounded-xl border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    ...
                    <div>
                        ...
                        @if($team->logo)
                            <div class="mb-2">
                                <img src="{{ Storage::url($team->logo) }}" class="w-16 h-16 rounded-xl object-cover border" alt="">
                            </div>
                        @endif
                    ...
                    <div>
                        ...
                        <select name="players_per_team" x-model.number="perTeam" @change="resizePlayers()"
                                class="w-full rounded-xl border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                            <option value="2" {{ old('players_per_team', $team->players_per_team) == 2 ? 'selected' : '' }}>2 pret 2</option>
                            <option value="4" {{ old('players_per_team', $team->players_per_team) == 4 ? 'selected' : '' }}>4 pret 4</option>
                            <option value="6" {{ old('players_per_team', $team->players_per_team) == 6 ? 'selected' : '' }}>6 pret 6</option>
                        </select>
                    </div>
