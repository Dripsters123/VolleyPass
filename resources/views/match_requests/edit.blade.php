<x-app-layout>
  ...
  <div class="max-w-3xl mx-auto p-6">
    ...
    <form method="POST" action="{{ route('match_requests.update', $request->id) }}" class="space-y-6" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input name="home_team" value="{{ old('home_team', $request->home_team) }}" class="w-full p-2 border rounded" required placeholder="Piem.: Rīgas Vilki">
        ...
        <div>
          ...
          <input name="away_team" value="{{ old('away_team', $request->away_team) }}" class="w-full p-2 border rounded" required placeholder="Piem.: Jūrmalas Viļņi">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input name="home_coach" value="{{ old('home_coach', $request->home_coach) }}" class="w-full p-2 border rounded" placeholder="Piem.: Jānis Bērziņš">
        </div>
        <div>
          <label class="block text-sm font-medium">Viesu treneris</label>
          <input name="away_coach" value="{{ old('away_coach', $request->away_coach) }}" class="w-full p-2 border rounded" placeholder="Piem.: Pēteris Kalniņš">
        </div>
      </div>

      <div>
        <label class="block text-sm font-medium">Tiesneši (komatu atdalīti)</label>
        <input name="judges" value="{{ old('judges', is_array($request->judges) ? implode(', ', $request->judges) : ($request->judges ?? '')) }}" class="w-full p-2 border rounded" placeholder="Piem.: Anna Ozola, Kārlis Liepa">
      </div>

      <div>
        <label class="block text-sm font-medium">Vieta — pilna adrese</label>
        <input name="location" value="{{ old('location', $request->location) }}" class="w-full p-2 border rounded" placeholder="Piem.: Sporta iela 2, Rīga, LV-1001">
      </div>

      <div class="max-w-xs">
        <label class="block text-sm font-medium">Ieteicamā biļetes cena (EUR)</label>
        <input type="number" step="0.01" min="0" name="ticket_price" value="{{ old('ticket_price', $request->ticket_price) }}"
      ...
      <div class="max-w-xs">
        ...
        <select id="players_per_team" name="players_per_team" class="w-full p-2 border rounded" required>
          <option value="2" {{ old('players_per_team', $request->players_per_team) == 2 ? 'selected' : '' }}>2 pret 2</option>
          <option value="4" {{ old('players_per_team', $request->players_per_team) == 4 ? 'selected' : '' }}>4 pret 4</option>
          <option value="6" {{ old('players_per_team', $request->players_per_team) == 6 ? 'selected' : '' }}>6 pret 6</option>
        </select>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="datetime-local" name="start_time" value="{{ old('start_time', optional($request->start_time)->format('Y-m-d\\TH:i')) }}" class="w-full p-2 border rounded" required>
        ...
        <div>
          ...
          <input type="datetime-local" name="end_time" value="{{ old('end_time', optional($request->end_time)->format('Y-m-d\\TH:i')) }}" class="w-full p-2 border rounded" required>
          ...
          <div class="lg:col-span-1 bg-white rounded-3xl border border-gray-200 p-5 shadow-sm">
            ...
            <div class="mt-5 space-y-4">
              <div>
                ...
                <input id="arena_name" name="arena_name" value="{{ old('arena_name', $request->arena_name) }}" required class="w-full rounded-2xl border border-gray-300 px-4 py-3 focus:border-blue-500 focus:ring-blue-200 focus:ring-2" placeholder="Piem.: Rīgas Sporta Halles Sektors A">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Apraksts</label>
                <input id="arena_description" name="arena_description" value="{{ old('arena_description', $request->arena_description) }}" class="w-full rounded-2xl border border-gray-300 px-4 py-3 focus:border-blue-500 focus:ring-blue-200 focus:ring-2" placeholder="Pievienot aprakstu (pēc izvēles)">
              </div>
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">Platums (metri)</label>
                  <input type="number" step="0.1" min="1" name="arena_width_m" value="{{ old('arena_width_m', $request->arena_width_m) }}" class="w-full rounded-2xl border border-gray-300 px-4 py-3 focus:border-blue-500 focus:ring-blue-200 focus:ring-2" placeholder="Piem.: 30">
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">Garums (metri)</label>
                  <input type="number" step="0.1" min="1" name="arena_height_m" value="{{ old('arena_height_m', $request->arena_height_m) }}" class="w-full rounded-2xl border border-gray-300 px-4 py-3 focus:border-blue-500 focus:ring-blue-200 focus:ring-2" placeholder="Piem.: 18">
                </div>
              </div>
      ...
      <input type="hidden" name="arena_id" id="arena_id_input" value="{{ old('arena_id', $request->arena_id) }}">
      <input type="hidden" name="arena_layout" id="arena-layout-input" value='{{ old('arena_layout', $request->arena_layout) ? json_encode(old('arena_layout', $request->arena_layout)) : '' }}'>
      <input type="hidden" name="arena_elements" id="arena-elements-input" value='{{ old('arena_elements', $request->arena_elements) ? json_encode(old('arena_elements', $request->arena_elements)) : '' }}'>
      <input type="hidden" name="arena_width" id="arena_width_input" value="{{ old('arena_width', $request->arena_width ?? 1000) }}">
      <input type="hidden" name="arena_height" id="arena_height_input" value="{{ old('arena_height', $request->arena_height ?? 700) }}">

      <div class="flex items-center gap-3">
        <button class="bg-blue-600 text-white px-4 py-3 rounded-2xl font-semibold shadow hover:bg-blue-700">Saglabāt izmaiņas</button>
        <a href="{{ route('match_requests.my') }}" class="px-4 py-3 rounded-2xl border border-gray-300 text-gray-700 hover:bg-gray-100">Atcelt</a>
      </div>
    </form>
  </div>
